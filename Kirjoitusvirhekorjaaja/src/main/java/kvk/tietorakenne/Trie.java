...
/**
 * Toteuttaa Trie tietorakenteen suomen kielen sanastoa varten. Trien avulla yksittäisestä sanasta saadaan nopeasti selville kuuluuko se suomen kielen sanastoon vai ei.
 * 
 * Aikavaativuudet:
 * Lisäys O(m)
 * Joukkoon kuuluminen O(m)
 * Alustaminen O(nm)
 * , missä m on merkkijonon pituus ja n on lisättävien merkkijonojen määrä.
 * 
 * 
 * Tilavaatimus noin:
 * O(m*A*N), missä m on keskimääräinen merkkijonon pituus, A on aakkoston koko ja N on merkkijonojen määrä.
 * 
...
public class Trie {
    ...
    /**
     * Lisää Trie-puuhun uuden merkkijonon. Jos merkkijono on jo puussa, ei puu muutu.
     * Aikavaatimus O(m), missä m on lisättävän sanan pituus.
     * @param sana
     */
    ...
    /**
     * Tarkastaa onko Trie-puussa parametrina annettava merkkijono.
     * Aikavaatimus O(m), missä m on tarkistettavan merkkijonon pituus.
     * @param sana
